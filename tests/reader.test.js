  ...
  beforeEach(async () => {
    ...
    describe('with records in the database', () => {
        ...
        describe('GET /readers/:id', () => {
            xit('gets a reader record by id', async () => {
            ...
            xit('returns a 404 if the reader does not exist', async () => {
                ...
                expect(response.message).to.equal("Reader 9999999 does not exist.");
            });
        });
        
        describe('PATCH /readers/:id', () => {
            xit('updates reader email by id', async () => {
                const reader = readers[0];
                ...
                expect(response.status).to.equal(200);
                expect(updatedReaderRecord).to.equal('smitty@mclovin.com');
            });
            ...
            xit('returns a 404 if the reader does not exist', async () => {
                ...
                expect(response.status).to.equal(404);
                expect(response.message).to.equal('Reader 999999 does not exist.');
            });
        });
